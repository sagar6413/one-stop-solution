...
sequenceDiagram
    ...
    Frontend->>+GraphQL API: Send registration details (GraphQL mutation)
    ...
    GraphQL API-->>-Frontend: Registration success response
    Frontend-->>-User: Registration successful

    %% Login Process
    User->>+Frontend: Enter login credentials
    Frontend->>+Keycloak: Send login credentials
    Keycloak-->>-Frontend: Return access token
    Frontend->>+GraphQL API: Send access token (GraphQL mutation)
    GraphQL API->>+Keycloak: Validate token
    Keycloak-->>-GraphQL API: Token valid
